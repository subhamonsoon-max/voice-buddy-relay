...
async def summarize_session(
    ...
    """Uses Gemini Flash to extract a summary and new facts from the session transcript."""
    if not transcript or not transcript.strip():
        return SessionSummaryResult(summary="Empty session with no spoken turns.", new_facts=[])

    key = api_key or settings.GEMINI_API_KEY
    if not key:
        logger.warning("No GEMINI_API_KEY provided. Returning mock summary.")
        return SessionSummaryResult(
            summary=f"Brief conversation ({len(transcript.splitlines())} turns).",
            new_facts=[],
        )
